...
function fromRow(r) {
  if (!r) return null
  return {
    petId: r.pet_id, sex: r.sex, neutered: r.neutered,
    neuteredAge: r.neutered_age, bloodType: r.blood_type,
    allergies: r.allergies ?? [],
    conditions: r.conditions ?? [],
    surgeries: r.surgeries ?? [],
    environment: r.environment,
    livingWithAnimals: r.living_with_animals,
    behavioralNotes: r.behavioral_notes,
    vetQuestions: r.vet_questions,
...
function buildDefault(petId) {
  return {
    petId, sex: 'unknown', neutered: null, neuteredAge: null,
    bloodType: null, allergies: [], conditions: [], surgeries: [],
    environment: null, livingWithAnimals: null,
    behavioralNotes: null, vetQuestions: null, updatedAt: null,
  }
}
  ...
  async upsert(petId, data) {
    const [row] = await sql`
      INSERT INTO medical_profiles
        (pet_id, sex, neutered, neutered_age, blood_type, allergies,
         conditions, surgeries, environment, living_with_animals,
         behavioral_notes, vet_questions, updated_at)
      VALUES
        (${petId}, ${data.sex ?? 'unknown'}, ${data.neutered ?? null},
         ${data.neuteredAge ?? null}, ${data.bloodType ?? null},
         ${data.allergies ?? []}::text[],
         ${JSON.stringify(data.conditions ?? [])}::jsonb,
         ${JSON.stringify(data.surgeries ?? [])}::jsonb,
         ${data.environment ?? null}, ${data.livingWithAnimals ?? null},
         ${data.behavioralNotes ?? null}, ${data.vetQuestions ?? null},
         NOW())
