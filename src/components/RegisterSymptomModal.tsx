...
export default function RegisterSymptomModal({ isOpen, onClose, onAdd, defaultPetId }: Props) {
  ...
  const handleSubmit = () => {
    if (!description.trim()) {
      setDescErr(t('symptoms.errDescription'))
      return
    }
      ...
      {success ? (
        ...
        <>
          ...
          <div className="form-group">
            ...
            <div
              className={['form-input', descErr ? 'form-input--err' : ''].join(' ')}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '.375rem', marginBottom: '1rem' }}>
            {SEVERITIES.map((s) => (
              <button
                ...
                style={{
                  borderColor: severity === s.val ? SEV_FG[s.val] : 'var(--border)',
                  background: severity === s.val ? SEV_BG[s.val] : 'var(--surface-offset)',
                }}
                onClick={() => setSeverity(s.val)}
              >
                <span style={{ fontSize: '1.1rem' }}>{SEV_ICON[s.val]}</span>
                <div style={{ flex: 1, textAlign: 'left' }}>
                  <div style={{ fontWeight: 800, fontSize: '.875rem', color: severity === s.val ? SEV_FG[s.val] : 'var(--text)' }}>
                    {s.label}
                  </div>
                  <div style={{ fontSize: '.75rem', color: 'var(--text-muted)' }}>{s.sub}</div>
                </div>
                <div
                  className={['mf-radio', severity === s.val ? 'checked' : ''].join(' ')}
          ...
          <div className="form-group" style={{ marginBottom: 0, marginTop: '.75rem' }}>
            <label className="form-label">
              {t('field.notes')} <span style={{ color: 'var(--text-faint)', fontWeight: 500 }}>({t('btn.optional')})</span>
            </label>
