    ...
    <div className="detail-overlay" onClick={onClose}>
      <div className="detail-sheet" onClick={e => e.stopPropagation()}>
        <div className="detail-header">
          <div className="detail-icon" style={{ background: SEV_BG[sev] || 'var(--err-hl)', color: SEV_COLOR[sev] || 'var(--err)', fontSize: '1.375rem' }}>
            {CAT_ICON[cat] || '🌡️'}
          </div>
...
export function EditSymptomModal({ isOpen, onClose, symptom, onSave }: EditProps) {
  ...
  useEffect(() => {
    if (symptom && isOpen) {
      setDescription(symptom.description); setCategory(symptom.category)
      setSeverity(symptom.severity); setDate(symptom.date); setNotes(symptom.notes)
      setDescErr('')
    }
  }, [symptom, isOpen])
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Editar síntoma" icon="🌡️"
      accentBg="var(--err-hl)" accentFg="var(--err)" size="md"
      footer={<PfFooter>
  <PfBtn variant="save" onClick={handleSave}>Guardar cambios</PfBtn>
      ...
      <div className="modal-hero" style={{ background: `linear-gradient(135deg,${selSev.bg},var(--surface))` }}>
        <div className="modal-hero-icon" style={{ background: selSev.fg, fontSize: '1.5rem' }}>{CAT_ICON[category]}</div>
        <div><div className="modal-hero-title">Editar síntoma</div><div className="modal-hero-sub">{PET_EMOJI[symptom.petId]} {PET_NAME[symptom.petId]}</div></div>
      </div>

      <div className="modal-section">Descripción</div>
      <div className="form-group">
        <div className={['form-input', descErr ? 'form-input--err' : ''].join(' ')} style={{ padding: 0 }}>
          <textarea style={{ width:'100%', padding:'.55rem .875rem', border:'none', background:'transparent', outline:'none', fontFamily:'inherit', fontSize:'.875rem', resize:'vertical', minHeight:72, color:'var(--text)' }}
            value={description} onChange={e => { setDescription(e.target.value); setDescErr('') }} autoFocus/>
        </div>
      ...
      <div className="symptom-cat-grid" style={{ marginBottom: '1rem' }}>
        {CATEGORIES.map(c => (
          <button key={c.val} type="button" className={['symptom-cat-btn', category===c.val?'active':''].join(' ')} onClick={() => setCategory(c.val)}>
      ...
      <div style={{ display:'flex', gap:'.375rem', marginBottom:'1rem', flexWrap:'wrap' }}>
        {SEVERITIES.map(s => (
          <button key={s.val} type="button"
            style={{ flex:1, padding:'.5rem .625rem', borderRadius:'var(--r-lg)', border:`1.5px solid ${severity===s.val?s.fg:'var(--border)'}`, background:severity===s.val?s.bg:'var(--surface-offset)', cursor:'pointer', fontWeight:700, fontSize:'.8125rem', color:severity===s.val?s.fg:'var(--text-muted)', fontFamily:'inherit', transition:'all var(--trans)', minWidth:70 }}
