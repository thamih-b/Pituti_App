...
async function apiPost(path: string, body: unknown) {
  ...
  if (!res.ok) throw new Error(data?.error ?? data?.message ?? 'Erro na autenticação')
          ...
          <div style={{ background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 'var(--r-xl)', padding: '2rem 1.75rem', boxShadow: 'var(--sh-lg)' }}>
            ...
            {mode === 'forgot' && (
              <div style={{ marginBottom: '1.25rem' }}>
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '.375rem', background: 'none', border: 'none', color: 'var(--primary)', fontWeight: 700, fontSize: '.875rem', cursor: 'pointer', fontFamily: 'inherit', marginBottom: '.875rem' }}
                >
                ...
                <div style={{ fontWeight: 800, fontSize: '1.25rem', color: 'var(--text)', marginBottom: '.375rem' }}>{t('login.forgotTitle')}</div>
                <div style={{ fontSize: '.875rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>{t('login.forgotSubtitle')}</div>
              </div>
            )}
            ...
            {success && mode === 'forgot' ? (
              <div style={{ textAlign: 'center', padding: '1rem 0' }}>
                ...
                <div style={{ fontWeight: 800, fontSize: '1.125rem', color: 'var(--text)', marginBottom: '.5rem' }}>{t('login.emailSentTitle')}</div>
                <div style={{ fontSize: '.875rem', color: 'var(--text-muted)', lineHeight: 1.5, marginBottom: '1.25rem' }}>
                  {t('login.emailSentBody', { email: `<strong style="color:var(--text)">${email}</strong>` })}
                </div>
                <button type="button" className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', minHeight: 48 }} onClick={() => switchMode('login')}>
                  {t('login.backToLogin')}
                </button>
              </div>
            ) : (
                ...
                >
                  {loading ? (
                    <>
                      ...
                      {mode === 'login' ? t('login.submittingLogin') : mode === 'register' ? t('login.submittingRegister') : t('login.submittingForgot')}
                    </>
                  ) : (
          ...
          {!success && (
            <p style={{ textAlign: 'center', marginTop: '1.25rem', fontSize: '.875rem', color: 'var(--text-muted)' }}>
              {mode === 'login' && (
                <>
                  {t('login.noAccount')}{' '}
                  <button type="button" onClick={() => switchMode('register')} style={{ background: 'none', border: 'none', color: 'var(--primary)', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', fontSize: 'inherit' }}>
              ...
              {mode === 'register' && (
                <>
                  {t('login.hasAccount')}{' '}
                  <button type="button" onClick={() => switchMode('login')} style={{ background: 'none', border: 'none', color: 'var(--primary)', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', fontSize: 'inherit' }}>
                    {t('login.signIn')}
                  </button>
                </>
              )}
