...
export default function MedicationsPage() {
  ...
  // FIX: seletor de pet para filtrar medicamentos
  ...
  const handleDelete = (id: string) => {
    ...
    if (editMed?.id === id) { setEditMed(null); setEditOpen(false) }
  }
  ...
  // FIX: filtro por pet selecionado
  const filteredActive  = selectedPetId ? active.filter(m => m.petId === selectedPetId)  : active
  const filteredHistory = selectedPetId ? history.filter(m => m.petId === selectedPetId) : history
  ...
  if (error) {
    return (
      <div>
        ...
        <div className="card" style={{ textAlign: 'center', padding: '3rem 2rem', color: 'var(--err)' }}>
          {error}
        </div>
      </div>
    )
    ...
    return (
      <div>
        ...
        <div className="card" style={{ textAlign: 'center', padding: '3rem 2rem' }}>
          ...
          <div style={{ fontWeight: 700, color: 'var(--text)', marginBottom: '.375rem' }}>{t('pets.noPets')}</div>
          <div style={{ fontSize: '.875rem', color: 'var(--text-muted)' }}>{t('pets.noPetsHint')}</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div className="grid-2">
        <div className="card">
          <div className="card-title">
            ...
            {filteredActive.length > 0 && <span className="badge badge-green">{filteredActive.length}</span>}
          </div>
        ...
        <div className="card">
          ...
          {filteredHistory.length === 0 ? (
            ...
            filteredHistory.map((m) => (
              <div key={m.id} className="list-item" style={{ opacity: 0.7, cursor: 'pointer' }} onClick={() => openDetail(m)}>
          ...
          {filteredActive.length === 0 ? (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '.5rem 0' }}>
              {(() => {
                ...
                const pct = total > 0
                  ? Math.round((filteredActive.filter(m => m.badgeCls === 'badge-green').length / total) * 100)
                  : 100
                ...
                return (
                  <svg width="90" height="90" viewBox="0 0 90 90" style={{ flexShrink: 0 }}>
                    ...
                    <text x="45" y="50" textAnchor="middle" fontFamily="Nunito,sans-serif"
                      fontWeight="800" fontSize="20" fill="var(--text)">{pct}%</text>
                  </svg>
                )
              })()}
                  ...
                  return (
                    <div key={m.id}>
                      ...
                      <div className="progress-wrap">
                        <div className={`progress-bar ${m.badgeCls === 'badge-green' ? 'success' : 'warn'}`}
                          style={{ width: `${pct}%` }} />
                      </div>
                      <div style={{ fontSize: '.7rem', color: m.badgeCls === 'badge-green' ? 'var(--success)' : 'var(--warn)', marginTop: '.2rem', fontWeight: 700 }}>
                        {days !== null ? `${t('medications.nextDose')} ${days}d` : `${t('vaccines.upToDate')} ✓`}
                      </div>
                    </div>
                  )
                })}
      ...
      {/* FIX: onAdd agora chama addMedication com os dados do form */}
      <AddMedicationModal
        isOpen={addOpen}
