...
const INITIAL_DOSES: DoseRecord[] = [
  {
    id: 'd1', medId: 'm2', icon: '💊',
    bg: 'var(--warn-hl)', color: 'var(--warn)',
    title: 'Pipeta antipulgas · Toby', date: '30 abr 2026',
    badge: '17d', badgeCls: 'badge-yellow', daysLeft: 17,
  },
  {
    id: 'd2', medId: 'm1', icon: '💊',
    bg: 'var(--blue-hl)', color: 'var(--blue)',
    title: 'Bravecto · Luna', date: '10 jul 2026',
    badge: '89d', badgeCls: 'badge-green', daysLeft: 89,
  },
]
...
export default function MedicationsPage() {
  ...
  const handleDelete = (id: string) => {
    deleteMedication(id)
    showToast(t.btn.delete)
  }

  const handleArchive = (id: string) => {
    archiveMedication(id)
    showToast(t.btn.archive)
  }

  const handleUnarchive = (id: string) => {
    unarchiveMedication(id)
    showToast(t.btn.unarchive)
  }
        ...
        <div className="card">
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'1.5rem', padding:'.5rem 0' }}>
            ...
            <div style={{ flex:1, display:'flex', flexDirection:'column', gap:'.625rem' }}>
              {[
                { label:'BRAVECTO — LUNA', pct:100, color:'success', sub:`${t.vaccines.upToDate} ✓`, subColor:'var(--success)' },
                { label:'PIPETA — TOBY',   pct:70,  color:'warn',    sub:`${t.medications.nextDoses} 17d`,   subColor:'var(--warn)'    },
              ].map((b) => (
                <div key={b.label}>
