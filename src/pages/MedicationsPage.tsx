...
const {
  active, history, loading, error, updateMedication,
  ...
  // ✅ derivado com fallback seguro
  const nextDoses = useMemo(() => {
    return active
      .filter((m) => !!m.endDate)
      .map((m) => {
        const endDate = m.endDate ?? ''
        const ms = new Date(endDate + 'T12:00:00').getTime() - Date.now()
  ...
  const canAddMedication = pets.length > 0
              ...
              >
                ...
                <div className="list-item-info">
                  ...
                  <div className="list-item-sub">
                    {m.dose}
                    {m.startDate
                      ? ` · ${new Date(m.startDate + 'T12:00:00').toLocaleDateString(t('dates.locale'), {
                          day: '2-digit',
                          month: 'short',
                          year: 'numeric',
                        })}`
                      : ''}
                    {m.endDate
                      ? ` → ${new Date(m.endDate + 'T12:00:00').toLocaleDateString(t('dates.locale'), {
                          day: '2-digit',
                          month: 'short',
                          year: 'numeric',
                        })}`
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '.5rem 0' }}>
              {(() => {
                ...
                const pct =
                  total > 0
                    ? Math.round(
                        (active.filter((m) => m.badgeCls === 'badge-green').length / total) * 100
                      )
                    : 100
                  ...
                  return (
                    <div key={m.id}>
                      ...
                      <div className="progress-wrap">
                        <div
                          className={`progress-bar ${
                            m.badgeCls === 'badge-green' ? 'success' : 'warn'
                          }`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                      <div
                        style={{
                          fontSize: '.7rem',
                          color: m.badgeCls === 'badge-green' ? 'var(--success)' : 'var(--warn)',
                          marginTop: '.2rem',
                          fontWeight: 700,
                        }}
                      >
        ...
        <div className="card">
          ...
          {nextDoses.length === 0 ? (
            ...
            nextDoses.map(({ med, days }) => (
              ...
              >
                <div className="list-item-icon" style={{ background: med.bg, color: med.color }}>
                  {med.icon}
                </div>
                <div className="list-item-info">
                  ...
                  <div className="list-item-sub">
                    {med.endDate
                      ? new Date(med.endDate + 'T12:00:00').toLocaleDateString(t('dates.locale'), {
                          day: '2-digit',
                          month: 'short',
                          year: 'numeric',
                        })
