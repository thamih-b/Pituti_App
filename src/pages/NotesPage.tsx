...
// TYPE_BADGE movido para dentro dos componentes que usam t
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop:'.75rem', display:'flex', alignItems:'center', gap:'.375rem', flexWrap:'wrap' }}>
        {note.authorName && (
          <div style={{
            ...
            <span style={{ fontSize:'.72rem', fontWeight:700, color:'var(--text-muted)' }}>
              {note.authorId === CURRENT_USER.id ? t.modal.caregiver : note.authorName}
            </span>
          </div>
        )}
        ...
        {archived && (
          <span className="badge badge-gray" style={{ opacity:.65 }}>📁 {t.notes.archived}</span>
        )}
      </div>
    </div>
  )
    ...
    <div>
      ...
      <div className="grid-auto">
        ...
        >
          ...
          <div className="note-add-card-sub">{t.notes.subtitle}</div>
        </div>
      </div>
